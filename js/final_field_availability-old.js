(function ($) {
    $(function () {
        ...
        if (is_spiel_form || is_newcomer_form) {
            ...
            var zugangs_codes_value_id = '#field-zugangscodes-values';
            ...
            var no_access_codes_unchecked = ['#edit-field-art-des-codes',
                '#edit-field-zugangscodes'];
            //Others
            var einreichung_abschliessen_id = '#edit-field-einreichung-abgeschlossen';
            ...
            add_fields_dependencies_fields_values_types_ftp_server_is_0.push(['text', 'text', 'url', 'text']);
            ...
            all_needs_to_set_always_ids.push('#edit-field-genre-und');
            ...
            all_needs_to_set_always_ids.push('#edit-field-plattformen-und');
            ...
            all_needs_to_set_check_type.push('choice');
            ...
            if (is_newcomer_form) {
                ...
                $('#newcomer-node-form').before('<p><div id="' + field_error_values_id + '" style="color:red"></div></p>');
                ...
                $('#spiel-node-form').before('<p><div id="' + field_error_values_id + '" style="color:red"></div></p>');
            }
            ...
            all_fields.forEach(function (v) {
                if (v == pictures_field_id) {
                    //All button from table of picture => all "Entfernen"-Button
    ...
    var areAllFieldAvalailable = function (all_needs_to_set_always_ids,
        ...
        if (
            ...
            ||
            ($(online_inhalte).is(":checked") && $(post_zusendung).is(":checked"))

        ) {
            count_set_field++;
            createAndShowElementErrorMessage(
                '.group-5-schritt',
                null,
                is_spiel_form,
                is_newcomer_form);
            ...
            createAndShowElementErrorMessage(
                '.group-5-schritt',
                'Fehlende Angabe über die Bereitstellung',
                is_spiel_form,
                is_newcomer_form);
        }
        ...
        if ($(online_inhalte).is(":checked")) {
            ...
            if ($(no_access_codes).is(":unchecked")) {
                ...
                curr_all_needs_to_set_check_type.push('choice_indirect');
            }
        }
            ...
            if (v == jury_preise_id && is_newcomer_form) {
                ...
                if (check_type == 'url') {
                    ...
                    if (counter >= number_of_needed_values) {
                        ...
                        createAndShowErrorMessage(
                            '#' + field_error_values_id,
                            curr_all_needs_to_set_always_ids[i], null,
                            is_spiel_form,
                            is_newcomer_form);
                    }
                    ...
                    if (v == jury_preise_id && is_spiel_form) {
                        var costs = 75 * $(jury_preise_id + ' input:checked').length;
    ...
    var showEinreichenAbschliessenIfAllAvailable = function (all_needs_to_set_always_ids,
        ...
        $(zugangs_codes_value_id + ' .form-text').each(function (i, v) {
            $(v).change(function () {
                console.log(1);
                showEinreichenAbschliessenIfAllAvailable(
                    all_needs_to_set_always_ids,
        ...
        var result = areAllFieldAvalailable(
            ...
            jury_preise_id);
        if (result == 100) {
            $(einreichung_abschliessen_id).show();
        } else {
            $(einreichung_abschliessen_id).hide();
        }
    ...
    var convertIdToUserText = function (field_id,
        ...
        sourceField.push('edit-field-genre-und');
        ...
        sourceField.push('edit-field-plattformen-und');
    ...
    var createAndShowErrorMessage = function (field_error_values_id,
        ...
        if (field_id) {
            ...
            if (userMessage) {
                if ($(field_error_values_id).html()) {
                    $(field_error_values_id).html($(field_error_values_id).html() + '<br/>' + userMessage);
                } else {
                    $(field_error_values_id).html($(field_error_values_id).html() + userMessage);
                }
            }
        } else if (basicString) {
            $(field_error_values_id).html($(field_error_values_id).html() + '<br/>' + basicString);
        } else {
            $(field_error_values_id).html('Fehler in der Anwendung. Bitte wenden Sie sich an den Administrator.');
        }
    };
    ...
    var createAndShowElementErrorMessage = function (field_id,
        ...
        if (field_id) {
            ...
            if (basicString) {
                $(field_id).prepend('<p id="'+errorElement+'" style="color:red;">' + basicString + '</p>');
            } else {
